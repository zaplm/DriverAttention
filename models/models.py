...
class ConvNextModel(nn.Module):
    def __init__(self):
        ...
        self.linear_upsampling = nn.UpsamplingBilinear2d(scale_factor=2)
        self.deconv_layer0 = nn.Sequential(
            nn.Conv2d(in_channels=768, out_channels=384, kernel_size=3, padding=1, bias=True),
            nn.ReLU(inplace=True),
            self.linear_upsampling
        )
        self.deconv_layer1 = nn.Sequential(
            nn.Conv2d(in_channels=768, out_channels=192, kernel_size=3, padding=1, bias=True),
            nn.ReLU(inplace=True),
            self.linear_upsampling
        )
        self.deconv_layer2 = nn.Sequential(
            nn.Conv2d(in_channels=384, out_channels=96, kernel_size=3, padding=1, bias=True),
            nn.ReLU(inplace=True),
            self.linear_upsampling
        )
        self.deconv_layer3 = nn.Sequential(
    ...
    def forward(self, x):
        ...
        # inputs = [ps.squeeze(1) for ps in inputs]
        # for j in range(len(inputs)):
        for i in range(8):
            x = self.convNext.features[i](x)
            # if i in [0, 1, 3]:
            #     results.append(x)
            if i % 2 == 1:
                outs.append(x)
        # inputs = torch.stack(inputs, dim=1)
        # _, inputs = self.convlstm(inputs)
        # x = inputs[0][0]
