...
class MobileViT(nn.Module):
    ...
    def __init__(self, model_cfg: Dict, num_classes: int = 1000):
        ...
        #     blocks.append(ConvLayer(in_channels=image_channels, out_channels=out_channels, kernel_size=i))
        # self.first_conv = nn.ModuleList(blocks)
        ...
        self.deconv_layer0 = nn.Sequential(
            nn.Conv2d(in_channels=160, out_channels=128, kernel_size=3, padding=1, bias=True),
            nn.ReLU(inplace=True),
            self.linear_upsampling
        )
        self.deconv_layer1 = nn.Sequential(
            nn.Conv2d(in_channels=256, out_channels=96, kernel_size=3, padding=1, bias=True),
            nn.ReLU(inplace=True),
            self.linear_upsampling
        )
    ...
    def forward(self, x: Tensor):
        ...
        #     features.append(m(x))
        # out0 = torch.cat([*features], dim=1)
        x = self.conv_1(x)
        out1 = self.layer_1(x) #32 112
        out2 = self.layer_2(out1) #64 56
        out3 = self.layer_3(out2) #96 28
        out4 = self.layer_4(out3) #128 14
        out5 = self.layer_5(out4) #160 7
