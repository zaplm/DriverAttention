...
if __name__ == '__main__':
    ...
    dataset = SceneDataset("/mapnew/xiaoyangbi/atten_data/driver_train_test/bd", mode='train', p_dic = ['ml_p', 'unisal_p'])
    # (img, img_c), (p, p_c) = dataset[0]
    img, img_c, p, p_c = dataset[0]
    
    # print(type(item))  # Should show tuple
    # print(len(item)) 
    
    # img, img_c = imgs # First element, expecting it to be [img, img_c]
    # p, p_c = ps  
    # import pdb; pdb.set_trace()
    print(img.shape) 
    print(img_c.shape)
